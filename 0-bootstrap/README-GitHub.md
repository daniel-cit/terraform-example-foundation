...
The objective of the following instructions is to configure the infrastructure that allows you to run CI/CD deployments using
GitHub Actions for the Terraform Example Foundation stages (`0-bootstrap`, `1-org`, `2-environments`, `3-networks`, `4-projects`).

The infrastructure consists in two Google Cloud Platform projects, `prj-b-seed` and `prj-b-cicd-wif-gh`, and associated resources.

It is a best practice to have two separate projects,`prj-b-seed` and `prj-b-cicd-wif-gh`, for separation of concerns.

The `prj-b-seed` project stores the terraform states generated by the terraform configurations of each stage and has the Service Accounts that are able to create / modify the infrastructure.

The `prj-b-cicd-wif-gh` project hosts the authentication infrastructure using [Workload identity federation](https://cloud.google.com/iam/docs/workload-identity-federation) and optionally the Google Kubernetes Engine infrastructure needed to run self-hosted runners for GitHub Actions using [Actions Runner Controller](https://github.com/actions/actions-runner-controller/blob/master/README.md).
