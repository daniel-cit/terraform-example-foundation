...
Run `terraform output cloudbuild_project_id` in the `0-bootstrap` folder to get the Cloud Build Project ID.
   ...
   **Note:** `gcp-policies` repo has the same name as the repo created in step `1-org`. In order to prevent a collision, the previous command will clone this repo in the folder `gcp-policies-project-infra`.
...
1. Navigate into the repo and copy contents of policy-library to new repo. All subsequent steps assume you are running them
   from the gcp-policies-project-infra directory. If you run them from another directory,
   adjust your copy paths accordingly.

   ```bash
   cd gcp-policies-project-infra
   git checkout -b main
...
1. Update `backend.tf` with your bucket from the infra pipeline output.
   ...
   for i in `find . -name 'backend.tf'`; do sed -i'' -e "s/UPDATE_PROJECT_INFRA_BUCKET/${backend_bucket}/" $i; done
   ```
