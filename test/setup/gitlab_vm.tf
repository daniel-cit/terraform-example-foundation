...
locals {
  ...
  setup_project_number           = module.project.project_number
...
resource "google_project_iam_member" "sd_viewer" {
  ...
  member  = "serviceAccount:service-${local.setup_project_number}@gcp-sa-cloudbuild.iam.gserviceaccount.com"
}
...
resource "google_project_iam_member" "cb_sa_pool_user" {
  project = module.gitlab_project.project_id
  role    = "roles/cloudbuild.workerPoolUser"
  member  = "serviceAccount:${local.setup_project_number}@cloudbuild.gserviceaccount.com"
}
