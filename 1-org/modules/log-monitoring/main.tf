...
locals {
  ...
  log_source_name = var.random_suffix ? "Log Monitoring - ${random_id.suffix.hex}" : "Log Monitoring"
}
...
resource "google_artifact_registry_repository" "cloudfunction" {
  ...
  project       = var.project_id
  repository_id = "ar-log-monitoring-${random_id.suffix.hex}"
...
data "archive_file" "function_source_zip" {
  ...
  output_path = "${path.module}/log_monitoring.zip"
...
module "cloudfunction_source_bucket" {
  ...
  project_id    = var.project_id
  name          = "bkt-log-monitoring-${random_id.suffix.hex}-sources-${data.google_project.project.number}-${var.location}"
  location      = var.location
...
resource "google_storage_bucket_object" "cf_log_source_zip" {
  name   = "${path.module}/log_monitoring.zip"
...
// SCC source
resource "google_scc_source" "log_monitoring" {
  display_name = local.log_source_name
  organization = var.org_id
  description  = "SCC Finding Source for logMonitoring Cloud Functions."
}
...
module "cloud_function" {
  ...
  depends_on = [
    google_storage_bucket_object.cf_log_source_zip,
    time_sleep.wait_kms_iam
  ]
}
